...
2. 在当天卡片的 **策划** 或 **程序** 区域点 **＋ 添加我的一行**。分区标题为 24px 加粗，姓名自动带入；填写晨会内容后点右侧 **重新保存**。
...
每行按人员、晨会内容、操作排列，右侧“重新保存 / 删除本行”并排；内容框默认一行，长内容自动增高，记录间距为 4px。每张卡片最多 20 行、每行最多 300 字，同时受整卡容量限制。输入时不保存，“正在保存”只表示操作已接收，实际成功后本行刷新；按钮名称固定为“重新保存”。他人可能能点入客户端输入框，但无法保存或删除你的行。
...
一个官方 SDK 长连接服务所有配置群，回调核对实际群、当天消息、操作者、行归属与版本。布局 v6 每行一个水平根表单，姓名为固定宽度人员引用，内容占剩余宽度，两个按钮紧邻右侧；服务端只保存实际操作者该行字段，不保存或输出原始回调。客户端不设置必填以便删除空行；保存仍由服务端校验本人内容非空及长度。使用原生水平排列减少容器数量，保留整卡 200 元素与 20 行预算。

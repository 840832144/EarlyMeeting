...
- **[公司服务器技术接手](docs/LINUX_SYSTEMD.md)**：已于2026-09-15切换，实际运行独立Node + 用户级systemd；两个群原卡继续使用，Windows已停止并锁定启动。完整源码在`codex/task-0028-local-callback` / PR #4，当前main不是完整应用。
- [Docker Compose部署与通用切换](docs/LINUX_HANDOFF.md)：保留给具备Docker条件的服务器；不能与当前服务同时运行。
- **[一键启停、维护日志与每日归档](docs/LINUX_OPERATIONS.md)**：已部署，日志位于项目内；9月16日起按群归档成功提交的记录。
...
长连接与本人行填写已接通，策划/程序提交后显示文字、编辑后可重提；今日交付在正式群1手填、正式群2按已提交记录自动汇总。当前启用两个正式群，测试群关闭；周一至周五北京时间09:30每群每天一张；当前由Linux服务运行，Windows一键入口已锁定，正常使用不再依赖这台Windows电脑开机。10:15催交提醒暂停。多人输入反馈的稳定组件标识补丁已热更，提交红框、编辑蓝框；真实客户端草稿保留仍待使用反馈，不把接口成功当作缺陷已解决。正式代码等待Review。

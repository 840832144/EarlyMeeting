...
长连接与本人行填写已接通，当前卡片效果已获 User 明确验收。多群独立发卡改为工作日北京时间 09:45，每群每天一张；当前只配置测试群，正式群等 User 指定后接入，程序需在本机持续运行。10:15 指定名单催交提醒已暂停，暂不实施。按 User 要求只做核心功能，不增加复杂测试或验收，正式代码等待 Review。

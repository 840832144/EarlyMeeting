...
更新时间：2026-09-07。TASK-0028 / EARLYMEETING；续接正式群2预填与协作权限，代码已部署，名单匹配等待群组只读权限生效。Task In Progress，不标记 Done。
...
| 行权限 | 仅正式群2允许本群所有人提交、删除任意行；其他群仍限本人。代填不改变原姓名，群/卡片/版本校验保留，隐藏他人按钮未实现 |
| 预填开关 | 新群/日期卡片可按策划/程序名单预建空行，已有卡片及删除结果不重建；正式群2待匹配3+6人，开关暂时关闭 |
| 名单权限 | 只读核对群2成员实际返回 99991672，需 im:chat:readonly；等待 User 开通生效，不申请部门/组织架构权限，不猜 ID |
